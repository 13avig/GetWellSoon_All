...
// start the app with endpoint "/clinicalgovtrials"

var router = express.Router();

router.use(function printUrl(req, res, next) {
    //console.log("Url: " + req.headers["trialurl"] + "TBody " + req.body);
    next();
})

router.route("/clinicalgovtrials")
    .post(function (req, res) {

        var args = req.body;
        let urlgiven = args['trialurl'];

        // request function to operate if no error
        request(urlgiven, function (error, response, html) {
            ...
            if (!error) {
                ...
                $("#tab-body").filter(function () {
                    ...
                    for (i = 0; i < common.find("tr").length; i++) {
